...
; INFOS
; Maggie is a blitter extension, the DMA Blitter should be active
; Texture format is DDS only and the texture should be on a 8 bytes boundary
; Maggie works in 16 & 32 bits only
...
; DMA activation
DMA_SET             = DMA_ON|DMA_COPPER|DMA_BLITTER
DMA_SET2            = DMA_STOP
...
INT_SET             = INT_ON|INT_VERTB|INT_BLIT
...
TestMaggie:
; Simply draw our texture on the screen
  lea     CUSTOM,a6
.PrepareMaggie:
  move.l  #Texture+128,TEXTPT(a6)       ; The texture adr
  move.l  #ZBuffer,TEXZPT(a6)           ; Z buffer adr
  move.w  #8,TEXMIP(a6)                 ; Texture size (8=256)
  move.w  #MAGGIE_16BITS|MAGGIE_ZBUFFER,TEXMODE(a6)    ; Maggie mode (16bits output + Z buffer)
  move.w  #2,TEXDMODE(a6)               ; Texel size in byte (16bits = 2 bytes)

  move.l  PhysicalScreen,a0
  move.l  #0,d1
  move.w  #255,d7                       ; Our texture is 256x256
.NextSpan:

  move.l  a0,TEXDPT(a6)                 ; Start adr of the destination
  move.l  #$0,TEXUVAL(a6)               ; U value in fixed 16:16
  move.l  d1,TEXVVAL(a6)                ; V value in fixed 16:16
  move.l  #$10000,TEXDUVAL(a6)          ; Delta U in fixed 16:16
  move.l  #$0,TEXDVVAL(a6)              ; Delta V in fixed 16:16
  move.w  #$ffff,TEXLIGHT(a6)           ; Light value in fixed 8:8
  move.w  #$00,TEXDLIGHT(a6)            ; Delta ligth in fixed 8:8
  move.l  #$ffffff,TEXLCOLOR(a6)        ; Light color in ARGB
  move.l  #$200000,TEXZVAL(a6)          ; Z value in fixed 16:16
  move.l  #$8000,TEXDZVAL(a6)           ; Delta Z infixed 16:16
  move.w  #256,TEXSTRT(a6)              ; Line size & start Maggie

  adda.l  #SCREEN_WIDTH*2,a0            ; Next destination line
  addi.l  #$10000,d1                    ; Next texture line
  tst.b   DMACONR(a6)                   ; Wait for Maggie to finish the job
.WaitMaggie:
  btst    #6,DMACONR(a6)
  bne.s   .WaitMaggie
  dbf     d7,.NextSpan                  ; Copy the next line
  
; Draw a line from 0,260 to 479,260
  move.w  #$F81F,d0
  move.l  PhysicalScreen,a0             ; Our screen
  adda.l  #SCREEN_WIDTH*2*260,a0
  move.w  #SCREEN_WIDTH-1,d6            ; 640 pixels on a line
.NextPixel:
  move.w  d0,(a0)+                      ; Write our pixel value
  dbf     d6,.NextPixel
...
; Our texture aligned on a 8 bytes boundary
  CNOP    0,8
Texture:
  INCBIN  "TestTexture.dds"
...
ZBuffer:
  ds.w    SCREEN_WIDTH*SCREEN_HEIGHT
