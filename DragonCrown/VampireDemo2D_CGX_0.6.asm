...
;*           Lexo
;* Version:  0.6 (2017-07-18)
;* Compiler: Devpac 3.15
;* 
;******************************************************************************
;* 
;* LMB      Exit program
...
;* LEFT     Move sorceress to the left
;* RIGHT    Move sorceress to the right
;* UP       Move sorceress up
;* DOWN     Move sorceress down
;* A OR Q   Decrease number of sprites
;* Z OR W   Increase number of sprites
;* 
;******************************************************************************
;* 
;* TODO:
...
;******************************************************************************
;* 
;* Lexo : for CGX users, be sure to have a 16 bit mode in classic format
;*        R5G6B5 and not R5G6B5PC format (WinUAE users) otherwise the colors
;*        will be wrong
;* 
;******************************************************************************
...
VAMPIRE_MODE EQU 1                      ; Vampire target 0 = Yes, 1 = No
...
.AllocBuffers
  ...
  ELSE
  move.l   #SCREENSIZE,d0                 ; Screen Size * One Buffer
  ENDC
  ...
  ELSE
  move.l   #(MEMF_PUBLIC),d1              ; MemFlags
  ENDC
...
;------------------------------------------------------------------------------
  ...
  ELSE
  bsr      CopyFrameBuffer                ; Copy FB in CGX buffer
  ENDC
...
;******************************************************************************
	...
	tst.l    d0                             ; 
	beq      .LockError                     ; 
  move.l   FBAddr,a0                      ; Src buffer
  move.l   FBAddr1,d1                     ; Dest buffer
  move.l   FBBytesPerRow,d2
...
FBBytesPerRow DC.L 0                    ; FrameBuffer bytes per row
...
MyLockTagItems:                             ; Cybergraphics->LockBitMapTagList()
  ...
  DC.L LBMI_BYTESPERROW,FBBytesPerRow     ; FrameBuffer bytes per row
	DC.L 0,0                                ; TAGEND
...
;******************************************************************************
	...
	EVEN
VERSTRING:
	DC.B "$VER: VampireDemo2D 0.6 (2017-04-18) APOLLO-Team/Lexo",10,0,0;
